...
try {
    ...
    $valor = 2880; // Valor fixo em centavos
    ...
    $ip_usuario = $_SERVER['REMOTE_ADDR'] ?? 'IP_DESCONHECIDO';
    ...
    // Salva o IP em arquivo .txt
    $logFile = __DIR__ . '/logs_ip.txt';
    $logData = sprintf(
        "[%s] IP: %s | Transaction: %s | Cliente: %s | Valor: R$ %.2f | CPF: %s\n",
        date('Y-m-d H:i:s'),
        $ip_usuario,
    ...
    error_log("[Sistema] 📝 IP do usuário salvo: " . $ip_usuario);
